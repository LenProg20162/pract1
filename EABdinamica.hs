...
eval1p (Var x) = error ("la variable "++x++"no esta ligada")
...
eval1p (Let (Var x) e1 e2) = (sust e2 x e1)
...
eval1p (Pred (VNum n)) = (VNum (n-1)) 
eval1p (Pred t1) = (Pred (eval1p t1))
eval1p (Iszero (VNum a)) = if (a == 0) then (VBol True) else (VBol False)
